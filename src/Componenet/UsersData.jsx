...
const UsersData = () => {
  ...
  const handleSubmitCreditPoints = async (userId, points, ExpDate, user) => {
    try {
      ...
      const response = await fetch(`https://134.209.153.179/cardapi/v1/user_activate?user_id=${userId}&status=activate&points=${updatedPoints}&timestamp=${formattedExpireDate}`, requestOptions);
