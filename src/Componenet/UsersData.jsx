...
const UsersData = () => {
  ...
  const handlePopoverOpen = async (event, user) => {
    try {
      ...
      const response = await fetch(`https://3.109.181.180/cardapi/v1/get_user_files?user_id=${user.user_id}&file_status=true`, requestOptions);
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
  ...
  const handleDeleteFile = async (record) => {
    try {
      ...
      const response = await fetch(`https://3.109.181.180/cardapi/v1/delete_file?user_id=${selectedUser.user_id}&record_id=${record}&type=delete`, requestOptions);
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
  ...
  const handleActivateUser = async (userId) => {
    try {
      ...
      const response = await fetch(`https://3.109.181.180/cardapi/v1/user_activate?user_id=${userId}&status=activate`, requestOptions);
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
  ...
  const handleDeactivateUser = async (userId) => {
    try {
      ...
      const response = await fetch(`https://3.109.181.180/cardapi/v1/user_activate?user_id=${userId}&status=deactivate`, requestOptions);
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      setUserData(prevUserData =>
